#!/usr/bin/env python
"""
Script to create Hugo markdown files from Mastodon Toots

Taken and adapted from Brandon Rozek:
(https://github.com/Brandon-Rozek/website-toots)
"""
...
TOOT_CONTENT_LOCATION = "./data"
SERVER="https://indieweb.social"
# Quick way to find user id: https://prouser123.me/mastodon-userid-lookup/
MUID=112803627857659580
# Server default (when < 0) is 20
RETRIEVE_NUM_TOOTS=100000
...
def create_toot(toot_json, toot_type):
    """
    ...
    file.
    """
    toot_content = toot_json['reblog']['content'] \
        if toot_type == 'boost' else toot_json['content']
...
def toot_file_from_id(tootid, toottype):
    """Returns toot filename from id"""
    loc = f"{TOOT_CONTENT_LOCATION}/{toottype}/{tootid}.md"
    Path(loc).parent.mkdir(exist_ok=True, parents=True)
    return loc


def read_toot(tootid, toottype) -> Optional[Dict[Any, Any]]:
    """
    ...
    """
    try:
        with open(toot_file_from_id(tootid, toottype), "r", encoding="UTF-8") as toot_file:
...
def write_toot(toot, toottype):
    ...
    try:
        with open(toot_file_from_id(toot_id, toottype), "w", encoding="UTF-8") as toot_file:
            toot_file.write(create_toot(toot, toottype))
    except Exception as e:
...
toot_filenames = []
path = f"{TOOT_CONTENT_LOCATION}/**"
for path in glob(path, recursive=True):
    if '.md' in path:
        toot_filenames.append(path)
...
for stoot in server_toots:
    ...
    # save replies, boosts, and regular toots in separate folders
    if stoot['in_reply_to_id']:
        stoot_type = 'reply'
    elif stoot['content'] == '':
        stoot_type = 'boost'
    else:
    ...
    if stoot_id in toot_ids:
        saved_tootdata = read_toot(stoot_id, stoot_type)
